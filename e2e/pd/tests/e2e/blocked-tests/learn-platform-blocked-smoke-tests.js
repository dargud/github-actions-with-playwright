...
import { PDDetailPage } from "../../../page-objects/learn-platform/PDDetailPage";
...
test.describe("New web app Smoke suite", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await loginPage.login(user, false);
  ...
  test.skip("API: create a new user", async ({ page, request }) => {
    // TODO: move to the BeforeAll hook
    const loginPage = new LoginPage(page);
    ...
    const responseSecondStep = await request.post(
      `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${key.stage}`,
      {
        headers: {
    ...
    //Create a new user
    const responseUserData = await request.post(
      `${baseUrl.stage}/users/admin/user`,
      {
        headers: {
    ...
    // Add entitlements
    const responseEntitlementData = await request.post(
      `${baseUrl.stage}/entitlements/${pathId}/assign-to-user`,
      {
        headers: {
          Authorization: `Bearer ${idToken}`,
        },
    ...
    // Add a course
    const responseCourseData = await request.post(
      `${baseUrl.stage}/courses/admin/course/${pathId}/add-user`,
      {
        headers: {
          Authorization: `Bearer ${idToken}`,
        },
    ...
    // Add a pod
    const responsePodData = await request.put(
      `${baseUrl.stage}/courses/admin/add-user-to-pod`,
      {
        headers: {
          Authorization: `Bearer ${idToken}`,
        },
    ...
    const extractedLink = body.match(
      /https?:\/\/(\.)?\b([-a-zA-Z0-9()@:%_\+.~#?&//=;]*)/
    )[0];
    ...
    await expect(
      page.getByText("Thank you for verifying your account")
    ).toBeVisible();
  });
  ...
  test("The user can see the course details", async ({ page }) => {
    // We need to specify what exactly to check
    const onPDDetailPage = new PDDetailPage(page);
    await onPDDetailPage.isCourseDetailsDisplayed();
  });

  test("The user can see the Your Pod details", async ({ page }) => {
    // We need to specify what exactly to check
    const onPDDetailPage = new PDDetailPage(page);
    await onPDDetailPage.isCourseInstructorDetailsDisplayed();
  });

  test("The user can see the Pacing guide", async ({ page }) => {
    // We need to specify what exactly to check
    const onPDDetailPage = new PDDetailPage(page);
  ...
  test("The user can complete a single step of weekly task", async ({
    page,
  }) => {
    const onPDDetailPage = new PDDetailPage(page);
  ...
  test("The user can see a week status: Not complete", async ({ page }) => {
    await expect(
      page
        .locator(
          '[class="h-fit rounded-xl border px-4 text-sm border-error-extraDark bg-error-extraLight text-error-extraDark"]'
        )
        .first()
    ).toBeVisible();
  });
  ...
  test("The user can see a week status: Completed", async ({ page }) => {
    await expect(
      page
        .locator(
          '[class="h-fit rounded-xl border px-4 text-sm border-success-extraDark bg-success-extraLight text-success-extraDark"]'
        )
        .first()
    ).toBeVisible();
  });
