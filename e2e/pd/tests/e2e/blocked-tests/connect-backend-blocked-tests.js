...
import { SearchResultsPage } from "../../../page-objects/connect-backend/SearchResultsPage.js";
import { HeaderOfAnyPage } from "../../../page-objects/connect-backend/HeaderOfAnyPage.js";
import { AllCoursesPage } from "../../../page-objects/connect-backend/AllCoursesPage.js";
import { SingleCoursePage } from "../../../page-objects/connect-backend/SingleCoursePage.js";
import {
  SingleTutorialPage,
  TutorialPage,
} from "../../page-objects/connect-backend/SingleTutorialPage.js";
import { AllPathwaysPage } from "../../../page-objects/connect-backend/AllPathwaysPage.js";
import { SinglePathwayPage } from "../../page-objects/connect-backend/SinglePathwayPage.js";
import { TutorialWidget } from "../../../Widgets/TutorialWidget.js";
import { LivePage } from "../../../page-objects/connect-backend/LivePage.js";
import { AllProjectsPage } from "../../../page-objects/connect-backend/AllProjectsPage.js";
import { AllTutorialsPage } from "../../../page-objects/connect-backend/AllTutorialsPage.js";
import { LoginPage } from "../../page-objects/LoginPage.js";

test.describe("Connected BE: Smoke test suite, without logged in user", () => {
  const baseUrl = {
    devLogin: "",
    stageLogin: "",
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForURL(baseUrl.prodLogin);
  });
  ...
  test.skip("User can open EdLab Portal page", async ({ page }) => {
    // For some reasons test redirected to the Access Denied page
    test.slow(); //Delete after page performance improvements
    const welocomeScreen = new TutorialWidget(page);
    const onHeader = new HeaderOfAnyPage(page);

    // Close the Tutorial
    await welocomeScreen.isWelcomeScreenDisplayed();
    await welocomeScreen.clickDismissButton();

    // Open EdLab Portal page
    await onHeader.clickForEducators();
    await onHeader.clickForEducatorsItem("EdLab Portal");
  });

  test("User can open Partner Portal page", async ({ page }) => {
    test.slow(); //Delete after page performance improvements
    const welocomeScreen = new TutorialWidget(page);
    const onHeader = new HeaderOfAnyPage(page);
    ...
    // Open Partner Portal page
    await onHeader.clickForEducators();
    await onHeader.clickForEducatorsItem("Partner Portal");
  });
});

test.describe.skip(
  "Connected BE: Temporary skipped because of problems to clean the test data",
  () => {
    test("View and Start a Course @Smoke", async ({ page }) => {
      ...
      const onHeader = new HeaderOfAnyPage(page);
      const onAllCoursesPage = new AllCoursesPage(page);
      const onCoursePage = new SingleCoursePage(page);
      ...
      await onHeader.clickBrowseAndSelectItem("View all Courses");

      // Click on a the card image for any course not yet started
      await onAllCoursesPage.openCourse();
      const courseName = await page
        .locator('[class="title_1VxiLhbY"]')
        .first()
        .textContent();
      await page.locator('[class="card-content_1UQ-eWB2"]').first().click();
    ...
    test("Complete and evaluate a Tutorial @Smoke", async ({ page }) => {
      // Need to handle the test data deleting. A part ot steps are commented.
      ...
      await expect(
        page.locator('[class="browses-container_2-zC2B4X"]')
      ).toBeVisible({ timeout: 60000 });
      ...
      await page.waitForSelector('button[label="Mark step as completed"]');
      await page
        .locator('button[label="Mark step as completed"]')
        .first()
        .click();
      await expect(
        page.locator('[class="dot_n92J-EQX completed_3PTIgtA-"]')
      ).toBeVisible();
      // await page.waitForSelector('[class="dot_n92J-EQX completed_3PTIgtA-"]');
      ...
      await page
        .locator('button[label="Mark all complete and continue"]')
        .click();
      ...
      await onHeader.myLearningClick(page);
      await page
        .locator('button[class="tab_2SDQIqg1"]')
    ...
    test("Add a Comment @Smoke", async ({ page }) => {
      ...
      const courseName = await page
        .locator('[class="lesson-item_166RM3e5"]')
        .first()
      ...
      await page.waitForSelector(
        '[class="operate-item_2n9Aii70 comment_cJLWS9eS"]'
      );
      await page
        .locator('[class="operate-item_2n9Aii70 comment_cJLWS9eS"]')
        .click();
      await expect(
        page.locator('[class="drawer-container_3HcXgi5X"]')
      ).toBeVisible();
      ...
      await page.locator('[class="operate-item_3uNVhOzv"]').first().click();
      await expect(
        page.locator('[class="drawer-container_3HcXgi5X"]')
      ).toBeVisible();
      ...
      await page
        .getByRole("textbox", {
          name: "Ask a question or leave a comment on this step…",
        })
        .fill(
      ...
      await expect(
        page.locator('[class="dialog-contents_3sffEnR5"]')
      ).toBeVisible();
      await page.locator('button[label="Yes"]').click();
    });

    test.skip("Complete a Quiz @Smoke", async ({ page }) => {
      // Need the list of right answers here
    ...
    test.skip("Upload an Exercise Submission @Smoke", async ({ page }) => {
      // Temporarily blocked by Capcha
      ...
      await page.goto(
        "https://learn.unity.com/tutorial/challenge-sprite-pachinko?labelRequired=true&pathwayId=5f7bcab4edbc2a0023e9c38f&missionId=5f777d9bedbc2a001f6f5ec7"
      );
      await page.waitForSelector('button[label="Mark step as completed"]');
      // Mark all required steps complete that appear before the Submission step
      // await onTutorialPage.markAllStepsAsCompleted();
      await page
        .getByRole("button", { name: "Mark step as completed" })
        .first()
        .click();
      await expect(
        page.locator('button[label="Mark step as completed"]').first()
      ).not.toBeVisible();
      ...
      await page
        .locator('input[placeholder="Insert WebGL URL"]')
        .fill("https://play.unity.com/mg/fps/boomer-shooter");
      // await page.waitForSelector('[class="card-link_I-F39_bx"]');
      // await expect(page.locator('[class="card-link_I-F39_bx"]')).toBeVisible();
      await page
        .locator('textarea[limit="300"]')
        .fill("Some valid discription");
      await page.locator('[class="container_2vk1nNna"]').first().click();
    ...
    test("Resume a Pathway @Smoke", async ({ page }) => {
      ...
      await page.locator('button[label="Resume mission"]').first().click();
      await page.waitForSelector('button[label="Mark step as completed"]');
      await page
        .getByRole("button", { name: "Mark step as completed" })
        .first()
    ...
    test.skip("Complete a Pathway @Smoke", async ({ page }) => {
      // Temporarily blocked by Capcha
